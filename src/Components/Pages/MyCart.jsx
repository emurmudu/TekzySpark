...
const MyCart = () => {
    ...
    const fetchCart = async () => {
        try {
            const response = await fetch(`https://mission-10-server-eight.vercel.app/getCart`, {
                headers: {
    ...
    const handleDeleteProduct = async (cartItemId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(`https://mission-10-server-eight.vercel.app/deleteCartItem/${cartItemId}`, {
                        method: 'DELETE',
                        headers: {
                            'user-email': loggedInUserEmail,
                        },
